...
export function HistoricalAnalysisNotice() {
  ...
  return <div className="mb-6 flex flex-col gap-3 rounded-xl border border-accent/20 bg-accent/5 p-4 sm:flex-row sm:items-center sm:justify-between">
    <p className="flex min-w-0 flex-wrap items-center gap-2 text-sm"><History aria-hidden="true" className="size-4 shrink-0 text-accent" />Visualizando análise de {formatDateTime(viewingHistorical.created_at ?? viewingHistorical.data_analise)} <span className="max-w-full break-all text-muted">({viewingHistorical.id})</span></p>
